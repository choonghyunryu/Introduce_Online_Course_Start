...
코스 컨텐츠의 프론트 엔드(Front-End)는 개츠비 기반으로 구현되었습니다. 그러나 핸즈온을 위한 R 스크립트의 실행, 즉 백 엔드는(Back-End) R이 설치된 환경에서 실행됩니다. 이 환경은 Binder(mybinder.org)에서 도커 기반으로 구성됩니다. 이 구성을 다음과 같이 meta.json에 정의합니다.

"juniper" 섹션에서 변경이 필요한 "repo"와 "branch"만 변경하면 됩니다.  
...
개발한 Gihthub 코스 컨텐츠의 Github 리파지토리를 기술합니다. 필자는 템플리트 프로젝트를 "choonghyunryu/course-starter-dlookr"에 저장했기 때문이 이네스의 리파지토리에서 대체합니다. 그리고 branch는 "binder"에서 "master"로 대체합니다.
...
```{r, eval=FALSE, echo=TRUE}
https://mybinder.org/v2/gh/choonghyunryu/course-starter-dlookr/master
```
...
**branch**에는 **'master'**를 선택합니다. 그리고, 개츠비를 빌드할 설정은 netflity에서 설정한 기본 값을 그대로 사용합니다.
